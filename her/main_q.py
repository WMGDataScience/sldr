...
def init(config, agent='robot', her=False, object_Qfunc=None, 
    ...
    if 'Fetch' in ENV_NAME and 'Multi' in ENV_NAME and 'Flex' not in ENV_NAME:
        ...
        envs_test = SubprocVecEnv([make_env(ENV_NAME, i_env, 'Fetch') for i_env in range(1)])
        n_rob_actions = 4
        ...
        envs_test = SubprocVecEnv([make_env(ENV_NAME, i_env, 'Fetch') for i_env in range(1)])
        n_rob_actions = 4
        n_actions = (config['max_nb_objects']+1)//2 * len(config['obj_action_type']) + n_rob_actions
    ...
    experiment_args = ((envs,envs_test), memory, noise, config, normalizer, None)
