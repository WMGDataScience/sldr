...
def init(config, agent='robot', her=False, object_Qfunc=None, backward_dyn=None, object_policy=None, reward_fun=None):
    ...
    if config['agent_alg'] == 'PPO_BD':
        memory = (RolloutStorage(env._max_episode_steps-1, config['n_rollouts'], (observation_space,), action_space[0]),
...
def rollout(env, model, noise, normalizer=None, render=False, agent_id=0, ai_object=False, rob_policy=[0., 0.]):
    trajectories = []
...
def run(model, experiment_args, train=True):
    ...
    for i_episode in range(N_EPISODES):
        ...
        if train:
            for i_cycle in range(N_CYCLES):
                ...
                for i_rollout in range(N_ROLLOUTS):
                    ...
                    trajectory, _, _, _ = rollout(env, model, noise, normalizer, render=render, agent_id=agent_id, ai_object=ai_object, rob_policy=config['rob_policy'])
                ...
                if agent_id==0 and i_cycle%FT_RATE==FT_RATE-1:
                    ...
                    for i_rollout in range(N_ROLLOUTS):
                        render = config['render'] > 0 and i_rollout==N_ROLLOUTS-1
                        trajectories, _, _, _ = rollout(env, model, noise, normalizer, render=render, agent_id=agent_id, ai_object=True, rob_policy=config['rob_policy'])
        ...
        for i_rollout in range(N_TEST_ROLLOUTS):
            ...
            if config['agent_alg'] == 'PPO_BD':
                _, episode_reward, success, _ = rollout(env, model, noise=(False,False), normalizer=normalizer, render=render, agent_id=agent_id, ai_object=False, rob_policy=config['rob_policy'])
            else:
                _, episode_reward, success, _ = rollout(env, model, noise=False, normalizer=normalizer, render=render, agent_id=agent_id, ai_object=False, rob_policy=config['rob_policy'])
