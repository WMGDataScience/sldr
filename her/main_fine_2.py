...
from her.algorithms.ddpg_fine import DDPG_BD
        ...
        if train:
            for i_cycle in range(N_CYCLES):
                ...
                for i_rollout in range(N_ROLLOUTS):
                    ...
                    trajectories, _, _, _ = rollout(env, model, noise, normalizer, render=render, agent_id=agent_id, ai_object=ai_object, rob_policy=config['rob_policy'])
                ...
                if agent_id==0 and i_cycle%config['ai_object_fine_tune_rate']==config['ai_object_fine_tune_rate']-1:
                    ...
                    for i_rollout in range(N_ROLLOUTS):
                        render = config['render'] > 0 and i_rollout==N_ROLLOUTS-1
                        trajectories, _, _, _ = rollout(env, model, noise, normalizer, render=render, agent_id=agent_id, ai_object=True)
