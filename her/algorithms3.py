...
class MADDPG(object):
    ...
    def update_parameters(self, batch, normalizer=None):
        ...
        Q = self.critics[0](K.cat([s1,s2],dim=1), K.cat([a1,a2],dim=1))     
        V = self.critics_target[0](K.cat([s1_,s2_],dim=1), K.cat([a1_,a2_],dim=1)).detach()
        ...
        Q = self.critics[1](K.cat([s1,s2],dim=1), K.cat([a1,a2],dim=1))     
        V = self.critics_target[1](K.cat([s1_,s2_],dim=1), K.cat([a1_,a2_],dim=1)).detach()
        ...
        loss_actor = -self.critics[0](K.cat([s1,s2],dim=1), K.cat([a1,a2],dim=1)).mean()
        ...
        loss_actor = -self.critics[1](K.cat([s1,s2],dim=1), K.cat([a1,a2],dim=1)).mean()
