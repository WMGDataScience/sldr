...
def init(config, agent='robot', her=False, object_Qfunc=None, backward_dyn=None, object_policy=None, reward_fun=None):
    ...
    if 'Fetch' in ENV_NAME and 'Multi' in ENV_NAME:
        for i_env in range(N_ROLLOUTS):
            env.append(gym.make(ENV_NAME, n_objects=config['max_nb_objects'], obj_action_type=config['obj_action_type'], observe_obj_grp=config['observe_obj_grp']))
        n_rob_actions = 4
...
def rollout(env, model, noise, normalizer=None, render=False, agent_id=0, ai_object=False, i_env=0):
    trajectories = []
    ...
    for i_step in range(env[i_env]._max_episode_steps):
    ...
    for trajectory in trajectories:
        ...
        for i_step in range(env[i_env]._max_episode_steps):
            ...
            if (i_step < env[i_env]._max_episode_steps - 1): 
        ...
        if train:
            for i_cycle in range(N_CYCLES):
                ...
                for i_rollout in range(N_ROLLOUTS):
                    ...
                    trajectories, _, _, _ = rollout(env, model, noise, normalizer, render=render, agent_id=agent_id, ai_object=ai_object, i_env=i_rollout)
