...
class DDPG_BD(object):
    def __init__(self, observation_space, action_space, optimizer, Actor, Critic, loss_func, gamma, tau, out_func=K.sigmoid,
        ...
        self.rnd = RandomNetDist(observation_space).to(device)
    ...
    def update_parameters(self, batch, normalizer=None):
        ...
        r_intr = self.get_obj_reward(s1_)
        r_intr = normalizer[2].preprocess(r_intr)
    ...
    def update_rnd(self, batch, normalizer=None):
        ...
        target = self.rnd_target(s1_).detach()
        pred = self.rnd(s1_)
    ...
    def reward_fun(self, next_state):
        with K.no_grad():
            reward = K.pow((self.rnd_target(next_state.to(self.device)) - self.rnd(next_state.to(self.device))), 2).sum(1,keepdim=True)
        return reward
