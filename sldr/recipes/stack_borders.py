...
from sldr.main_multi import init as init_multi
from sldr.main_multi import run as run_multi
from sldr.main_multi_q import init as init_q
from sldr.main_multi_q import run as run_q
from sldr.main_multi_q_rnd import init as init_q_rnd
from sldr.main_multi_q_rnd import run as run_q_rnd
...
for n_objects in range(n_objects_lower,n_objects_higher):
    ...
    for i_exp in range(int(exp_config['start_n_exp']), int(exp_config['n_exp'])):
        ...
        path = './models_paper/batch3/' + rob_name + order_suffix + rew_suffix + str(i_exp)
        try:  
            os.makedirs(path)
        except OSError:  
        ...
        path = './models_paper/batch3/monitor_' + rob_name + order_suffix + rew_suffix + str(i_exp) + '.npy'
