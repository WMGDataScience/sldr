...
from sldr.algorithms.ddpg_fine import DDPG_BD
...
def run(model, experiment_args, train=True):
    ...
    for i_episode in range(N_EPISODES):
        ...
        if train:
            for i_cycle in range(N_CYCLES):
                ...
                for i_rollout in range(N_ROLLOUTS):
                    ...
                    trajectories, _, _, _ = rollout(env, model, noise, normalizer, render=render, agent_id=agent_id, ai_object=ai_object)
        ...
        for i_rollout in range(N_TEST_ROLLOUTS):
            ...
            _, episode_reward, success, _ = rollout(env, model, noise=(False,False), normalizer=normalizer, render=render, agent_id=agent_id, ai_object=False)
