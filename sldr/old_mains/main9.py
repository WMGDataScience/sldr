...
from sldr.algorithms5 import DDPG_BD
...
def rollout(env, model, noise, normalizer=None, render=False, agent_id=0):
    trajectories = []
    ...
    env.env.ai_object = True if agent_id==1 else False
    state_all = env.reset()
...
def run(model, experiment_args, train=True):
    ...
    for i_episode in range(N_EPISODES):
        ...
        if train:
            for i_cycle in range(N_CYCLES):
                ...
                for i_rollout in range(N_ROLLOUTS):
                    ...
                    trajectories, _, _, _ = rollout(env, model, noise, normalizer, render=(i_rollout==N_ROLLOUTS-1), agent_id=agent_id)
        ...
        for i_rollout in range(N_TEST_ROLLOUTS):
            ...
            _, episode_reward, success, _ = rollout(env, model, noise=False, normalizer=normalizer, render=render, agent_id=agent_id)
